...
import type { BacklinkRow, BacklinkTargetType, Page, PageTreeNode, PageVisibility } from "@/types/pages";
...
export async function fetchPageTree(rootId?: string): Promise<PageTreeNode[]> {
  if (IS_PREVIEW) {
    ...
    const walk = (parent: string | null, depth: number) => {
      mockPages
        .filter((p) => p.parent_id === parent && !p.archived_at && (rootId ? depth > 0 || p.id === rootId : true))
...
export async function fetchPage(id: string): Promise<Page | null> {
  if (IS_PREVIEW) return mockPages.find((p) => p.id === id) ?? null;
